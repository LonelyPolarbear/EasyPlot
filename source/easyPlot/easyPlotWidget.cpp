...
{
	{
		...
		std::cout << u8"[2]\n深度模板附件测试\n";
		{
			...
			XQ::print(u8"深度模板附件测试...");
			{
				...
				XQ::print(u8"cpu端预设深度模板");
				std::cout << std::hex;
				cpuDepthData->dump();
				std::cout << std::dec;

				XQ::print(u8"创建深度模板纹理");
				auto depthTexture = makeShareDbObject< XOpenGLTexture>();
