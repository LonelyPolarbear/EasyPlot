...
struct ConvertFun2FunPointer<object(Args...)>
{
	using pointerType = object * (*)(Args...);
	using sharedPointerType = std::shared_ptr<object>(*)(Args...);
	using product = object;
	using TList = tmpTypelist<Args...>;
	using return_type = object;
};
...
struct GenScatterHierarchy :public GenScatterHierarchy<identifier, type + 1, ctorTypes...>
{
	...
	using sharedPointerType = typename ConvertFun2FunPointer<ctorType>::sharedPointerType;
	...
	std::shared_ptr<product> DoCreateShare(IntToType<type>, identifier id, Args...values) {
		...
		if (AssoicMap.find(id) != AssoicMap.end()) {
			...
			if (sharedPointrMap.find(id) != sharedPointrMap.end()) {
				auto result = sharedPointrMap[id](values...);
				auto f = s_initFns[id];
				if (f != nullptr) {
					((*result).*f)();
				}
	...
	bool RegisterTofactory(IntToType<type>, identifier id, void(product::* f)() = nullptr) {
		std::lock_guard<std::mutex> lock(s_mutex);
		auto fun = &policy<product, ConcreateProduct, ctor_types>::crateSelf;
		//获取返回值类型
		using return_type = typename ConvertFun2FunPointer<decltype(policy<product, ConcreateProduct, ctor_types>::crateSelf)>::return_type;
		if constexpr (std::is_same_v<product*, return_type  >) {
			...
			//该函数直接返回智能指针
			if (sharedPointrMap.find(id) == sharedPointrMap.end()) {
				sharedPointrMap.insert(std::pair<identifier, sharedPointerType>(id, fun));
				s_initFns.insert(std::pair<identifier, void(product::*)()>(id, f));
				return true;
			}
		}
	...
	std::list<identifier> getIdentifiers(IntToType<type>) {
		...
		for (auto iter = sharedPointrMap.begin(); iter != sharedPointrMap.end(); iter++) {
			keys.push_back(iter->first);
		}
		return keys;
	}
public:
	...
	std::map<identifier, sharedPointerType> sharedPointrMap;
...
struct GenScatterHierarchy<identifier, type, ctorType>
{
	...
	bool RegisterTofactory(IntToType<type>, identifier id, void(product::* f)() = nullptr) {
		...
		auto fun = &policy<product, ConcreateProduct, ctor_types>::crateSelf;
		using return_type = typename ConvertFun2FunPointer<decltype(policy<product, ConcreateProduct, ctor_types>::crateSelf)>::return_type;
		if constexpr (std::is_same_v<product*, std::decay_t<return_type>  >) {
			if (AssoicMap.find(id) == AssoicMap.end()) {
				AssoicMap.insert(std::pair<identifier, pointerType>(id, fun));
				s_initFns.insert(std::pair<identifier, void(product::*)()>(id, f));
				return true;
