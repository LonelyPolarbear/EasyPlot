...
XColorComboBox::XColorComboBox(QWidget* parent) :QComboBox(parent)
{
	setEditable(false);
...
void XColorComboBox::enableColorDialog(bool enabled)
{
...
bool XColorComboBox::hasColor(const QColor& c)
{
	int num = count();
...
void XColorComboBox::appendColor(const QColor& c, const QString& n)
{
...
void XColorComboBox::appendOtherColor()
{
...
void XColorComboBox::updateOtherColor(const QColor& color)
{
	QPixmap pix(16, 16);
	pix.fill(Qt::transparent);
	QPainter painter(&pix);
...
void XColorComboBox::insertColor(const QColor& c, const QString& n, int i)
{
	if (!c.isValid() || hasColor(c)) return;
	int num = count();
	if (_colorDialogEnabled) num--;
...
QColor XColorComboBox::currentColor() const
{
	int index = currentIndex();
	return color(index);
}

void XColorComboBox::setCurrentColor(const QColor& c)
{
	bool existed = false;
...
QColor XColorComboBox::color(int index) const
{
...
void XColorComboBox::emitActivatedColor(int index)
{
	/* if(_colorDialogEnabled&&index==count()-1)
	{
...
void XColorComboBox::emitHighlightedColor(int index)
{
	if (!_colorDialogEnabled || index != count() - 1) emit highlighted(color(index));
}

QSize XColorComboBox::sizeHint() const
{
	QFontMetrics fm(font());
	return QSize(fm.width(tr("#RRGGBB")) + 16 + 4 + 20, fm.height() + 4);
}

void XColorComboBox::appendPredefinedColors()
{
...
void XColorComboBox::appendBackgroundColors()
{
	appendColor(Qt::black, tr("Black"));
	appendColor(Qt::gray, tr("Gray"));
	appendColor(Qt::white, tr("White"));
...
void XColorComboBox::clearAllColors()
{
	clear();
	if (_colorDialogEnabled) addItem(tr("More ..."));
}

bool XColorComboBox::colorDialogEnabled() const
{
