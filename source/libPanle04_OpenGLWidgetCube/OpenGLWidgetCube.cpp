...
struct OpenGLWidgetCube::Internal {
...
OpenGLWidgetCube::OpenGLWidgetCube(QWidget* parent) :XOpenGLWidget(parent),d(std::make_unique<Internal>())
{
	setFocusPolicy(Qt::StrongFocus); // 添加这行
}

OpenGLWidgetCube::~OpenGLWidgetCube()
{
}

void OpenGLWidgetCube::render()
{
	makeCurrent();
...
void OpenGLWidgetCube::initGLResource()
{
	d->shader = xshader(myUtilty::ShareVar::instance().currentExeDir + "/02cube/" + "cube.vs", myUtilty::ShareVar::instance().currentExeDir + "/02cube/" + "cube.fs");
	//!
	//! create VAO VBO EBO
...
void OpenGLWidgetCube::mousePressEvent(QMouseEvent* event)
{
...
void OpenGLWidgetCube::mouseMoveEvent(QMouseEvent* event)
{	
...
void OpenGLWidgetCube::mouseReleaseEvent(QMouseEvent* event)
{
...
void OpenGLWidgetCube::wheelEvent(QWheelEvent* event)
{	
	event->angleDelta();
...
void OpenGLWidgetCube::resizeEvent(QResizeEvent* event)
{
