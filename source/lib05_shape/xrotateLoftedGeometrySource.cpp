...
void XRotateLoftedgeometrySource::updateVertextCoordArray()
{
	//更新顶点
	//前面的四个坐标点
	std::vector<myUtilty::Vec2f> points;
	if (isClosed()) {
		points = myUtilty::math::circleSplit(m_NumVertices, m_angle);
	}
	else
		points = myUtilty::math::circleSplit(m_NumVertices - 1, m_angle);
...
//std::vector<myUtilty::Vec3f> XRotateLoftedgeometrySource::getBottomPoints() {
...
//}
//
//std::vector<myUtilty::Vec3f> XRotateLoftedgeometrySource::getTopPoints() {
//	std::vector<myUtilty::Vec3f> result;
//	std::vector<myUtilty::Vec2f> tmp;
//	if (isClosed()) {
//		tmp = myUtilty::math::circleSplit(m_NumVertices, m_angle);
