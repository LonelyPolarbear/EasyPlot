...
echo ================================
echo Generating template from !OUTPUT_FILE! with source file changed to "template.h" and output field empty
echo ================================

if "!OUTPUT_FILE!"=="" (
    echo [ERROR] OUTPUT_FILE is not set. Please set OUTPUT_FILE before running this script.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo [ERROR] Failed to generate template compile command from !OUTPUT_FILE!
    pause
    exit /b 1
)

echo Template saved to template_compile_command.json
echo.
